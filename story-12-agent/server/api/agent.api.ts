...
agentRouter.post('/query', async (req: Request, res: Response) => {
  try {
    const { sessionId, query, pageContext, siteConfig } = req.body as {
      sessionId: string;
      query: string;
    ...
    console.error('[AVA agent] REST error:', err);
...
export async function handleAgentWsMessage(
  ws: WebSocket,
  msg: Record<string, unknown>,
  _siteConfig?: SiteAdapterConfig,  // pass from existing session context if available
): Promise<void> {
  ...
  const pageContext = (msg.pageContext as PageContext) ?? {
    pageType: 'other',
    pageUrl: String(msg.siteUrl ?? ''),
  };

  const siteConfig = (_siteConfig ?? msg.siteConfig) as SiteAdapterConfig;
  ...
  try {
    ...
    console.error('[AVA agent] WS error:', err);
