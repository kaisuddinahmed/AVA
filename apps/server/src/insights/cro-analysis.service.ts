...
export interface CROFinding {
  ...
  suggestion: string; // plain-English fix from LLM
}
 ...
 */
export async function runCROAnalysis(siteUrl: string): Promise<CROFinding[]> {
  ...
  const groups = new Map<string, { frictionId: string; page: string; sessions: Set<string>; count: number }>();

  for (const e of frictionEvents) {
    if (!e.frictionId) continue;
  ...
  const findings: CROFinding[] = top.map((g, idx) => ({
...
async function generateCROSuggestions(
  items: Array<{ frictionId: string; page: string; count: number; sessions: Set<string> }>,
  siteUrl: string,
): Promise<string[]> {
  ...
  const list = items.map((g, i) =>
    `${i + 1}. ${g.frictionId} on ${g.page}: ${g.count} events in ${g.sessions.size} sessions`
  ).join("\n");
