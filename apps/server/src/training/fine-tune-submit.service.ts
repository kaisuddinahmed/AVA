...
import { formatAsFineTuningJsonl, type FormatterPreset } from "./training-formatter.service.js";
 ...
 */
export async function submitFineTuneJob(options: SubmitOptions): Promise<SubmitResult> {
  const {
    baseModel = config.groq.model,
    preset = "groq",
    maxExamples = 5000,
    siteUrl,
  } = options;
  ...
  // 4. Submit to Groq
  let fineTuneJobId: string | null = null;
  ...
  try {
    ...
    log.info(`[FineTune] Groq fine-tune submission not available: ${(err as Error).message}`);
...
export async function getFineTuneJobStatus(
  provider: string,
  jobId: string,
): Promise<{ status: string; fineTunedModel?: string; error?: string }> {
  if (provider !== "groq") {
  ...
  try {
    ...
    const job = await (groq as any).fineTuning.jobs.retrieve(jobId);
    ...
    return {
      status: "error",
      error: `Failed to poll Groq fine-tune job: ${(err as Error).message}`,
    };
  }
}
