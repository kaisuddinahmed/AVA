...
export interface UserAgentGroup {
  userAgents: string[]; // lower-cased; "*" for wildcard
  rules: RobotsRule[];
...
export interface ParsedRobots {
  ...
  rawUnknown: Array<{ line: number; key: string; value: string }>;
}
...
export function parseRobotsTxt(text: string): ParsedRobots {
  const result: ParsedRobots = { groups: [], sitemaps: [], rawUnknown: [] };
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (colonIdx < 1) continue; // malformed — silently drop
    ...
    if (key === "crawl-delay") {
      ...
      if (Number.isFinite(n) && n >= 0) currentGroup.crawlDelaySec = n;
      continue;
    }
...
export function isAllowed(parsed: ParsedRobots, userAgent: string, urlPath: string): boolean {
  ...
  for (const rule of group.rules) {
    if (!matchesPattern(rule.pattern, urlPath)) continue;
  ...
  if (bestLen < 0) return true; // no rule matched
  // Empty Disallow ("") is "allow everything" per RFC §2.2.2
  if (bestType === "disallow" && bestLen === 0) return true;
  return bestType === "allow";
}

/** Look up the crawl-delay (seconds) that applies to a given UA. */
export function crawlDelayFor(parsed: ParsedRobots, userAgent: string): number | undefined {
...
function pickGroup(parsed: ParsedRobots, userAgent: string): UserAgentGroup | undefined {
  ...
  let specific: UserAgentGroup | undefined;
  let specificLen = -1;
  let wildcard: UserAgentGroup | undefined;
