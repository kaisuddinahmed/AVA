...
export interface FingerprintResult {
  hash: string;                 // SHA-256 hex of canonical fingerprint JSON
 ...
 */
export function captureFingerprint(html: string, pageType: PageType): FingerprintResult {
...
function hashFingerprint(data: FingerprintData): string {
...
function isNoiseClass(cls: string): boolean {
  ...
  if (/^css-[a-z0-9-]+$/i.test(cls)) return true;       // emotion / CSS-in-JS
  if (/^sc-[a-z0-9]+/i.test(cls)) return true;           // styled-components
  if (/^__[\w]+_[a-f0-9]+$/i.test(cls)) return true;     // Next.js __className_HASH
  if (/^jsx-\d+$/i.test(cls)) return true;               // styled-jsx
...
function countTags(html: string): Record<string, number> {
...
function countClasses(html: string): Record<string, number> {
  const out: Record<string, number> = {};
  let m: RegExpExecArray | null;
  while ((m = CLASS_RE.exec(html)) !== null) {
    for (const tok of m[1].split(/\s+/)) {
      const cls = tok.trim().toLowerCase();
      if (!cls || isNoiseClass(cls)) continue;
      out[cls] = (out[cls] ?? 0) + 1;
    }
  }
  return out;
...
// ---------------------------------------------------------------------------
// Attribute signatures — stable structural markers worth fingerprinting
// ---------------------------------------------------------------------------
...
function collectAttrSignatures(html: string): string[] {
...
//
// Strategy: for each pageType, run an ordered list of candidate matchers
// against the HTML. Return the first matcher's selector value. If none
// fire, omit the key. Phase 1.5's LLM mapper fills gaps the heuristics
// can't reach.
// ---------------------------------------------------------------------------

interface SelectorRule {
  /** Key on InferredSelectors. */
  key: keyof InferredSelectors;
  /** Selector string to record when the test matches. */
  selector: string;
  /** Test against raw HTML. */
  test: (html: string) => boolean;
}

const PDP_RULES: SelectorRule[] = [
  // Add to cart
...
const CATEGORY_RULES: SelectorRule[] = [
...
function inferSelectors(html: string, pageType: PageType): InferredSelectors {
  const out: InferredSelectors = {};
  for (const rule of rulesFor(pageType)) {
    if (out[rule.key]) continue;          // first match wins
    if (rule.test(html)) out[rule.key] = rule.selector;
  }
  return out;
}
