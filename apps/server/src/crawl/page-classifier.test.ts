...
// ── Fixture matrix (Phase 1.0 gate) ─────────────────────────────────────────

interface FixtureCase {
  file: string;
  url: string;
  expectedType: PageType;
  /** Optional — assert at least one of these signals fired. */
  mustHaveSignal?: string;
}
...
describe("classifyPage — JSON-LD edges", () => {
  ...
  it("does not fire JSON-LD signal when no <script type='application/ld+json'> present", () => {
    const r = classifyPage("<html><body class='template-product'></body></html>",
                          "https://x.test/products/abc");
