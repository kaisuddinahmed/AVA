...
export type SitemapResult =
  ...
  | { kind: "sitemapindex"; sitemaps: ChildSitemap[] };
...
function iterBlocks(xml: string, tag: string): string[] {
  const re = new RegExp(`<(?:[\\w-]+:)?${tag}\\b[^>]*>[\\s\\S]*?<\\/(?:[\\w-]+:)?${tag}>`, "gi");
  return xml.match(re) ?? [];
}

function parseLastmod(raw: string | undefined): Date | undefined {
...
function parseChangefreq(raw: string | undefined): SitemapUrl["changefreq"] {
  if (!raw) return undefined;
 ...
 */
export function parseSitemap(xml: string): SitemapResult | null {
...
function parseIndex(xml: string): ChildSitemap[] {
