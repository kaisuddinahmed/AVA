...
function nodeProduct(overrides: Partial<{ id: string; handle: string; title: string; price: string; inv: number }> = {}): RawProductNode {
  return {
...
describe("Admin.listProducts — response transform", () => {
  it("captures status + totalInventory + inventoryQuantity (Storefront can't)", async () => {
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(
      jsonResponse(listProductsBody({ products: [nodeProduct({ inv: 7 })] })),
    );
    const r = await listProducts("x.myshopify.com", "tok", null, { fetchImpl: fetchMock });
    const p = r.products[0];
    expect<Partial<AdminProduct>>(p).toMatchObject({
      status: "ACTIVE",
      totalInventory: 7,
      currency: "USD",
    });
...
describe("ShopifyAdminError — kinds", () => {
  ...
  it("423 LOCKED → forbidden with status 423 (Shopify-locked shop)", async () => {
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(new Response("", { status: 423 }));
    try {
      await listProducts("x.myshopify.com", "tok", null, { fetchImpl: fetchMock });
      throw new Error("should have thrown");
    } catch (e) {
      expect(e).toBeInstanceOf(ShopifyAdminError);
      expect((e as ShopifyAdminError).kind).toBe("forbidden");
      expect((e as ShopifyAdminError).status).toBe(423);
    }
  });
  ...
  it("429 → rate_limited with retryAfterSec", async () => {
    ...
    try {
      ...
      expect((e as ShopifyAdminError).retryAfterSec).toBe(5);
    }
  });
...
describe("Admin.createDelegatedStorefrontToken", () => {
  it("mints a Storefront token via the Admin storefrontAccessTokenCreate mutation", async () => {
    ...
    const r = await createDelegatedStorefrontToken(
  ...
  it("throws graphql kind on userErrors", async () => {
    ...
    await expect(
      createDelegatedStorefrontToken("x.myshopify.com", "tok", "duplicate", { fetchImpl: fetchMock }),
    ).rejects.toMatchObject({ kind: "graphql" });
  });
  ...
  it("throws malformed_response when the token field is unexpectedly null without userErrors", async () => {
    ...
    await expect(
      createDelegatedStorefrontToken("x.myshopify.com", "tok", "AVA", { fetchImpl: fetchMock }),
  ...
  it("propagates 401 → unauthorized so the OAuth flow can re-prompt", async () => {
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(new Response("", { status: 401 }));
    await expect(
      createDelegatedStorefrontToken("x.myshopify.com", "bad", "AVA", { fetchImpl: fetchMock }),
    ).rejects.toMatchObject({ kind: "unauthorized" });
  });
});
...
function webhookCreateBody(opts: { id?: string | null; userErrors?: Array<{ field: string[] | null; message: string }> }) {
  return {
    data: {
...
describe("Admin.registerProductWebhooks", () => {
  it("subscribes to PRODUCTS_CREATE, PRODUCTS_UPDATE, PRODUCTS_DELETE with correct callback URLs", async () => {
    ...
    const fetchMock = vi.fn<typeof fetch>().mockImplementation(async () =>
      jsonResponse(webhookCreateBody({ id: ids[call++] })),
    );

    const result = await registerProductWebhooks(
      "https://example.myshopify.com",
      "shpat_admin",
      "https://app.ava.test",
      { fetchImpl: fetchMock },
    ...
    expect(bodies[1]!.variables.webhookSubscription.uri).toBe(
      "https://app.ava.test/api/shopify/webhooks/products/update",
    );
    expect(bodies[2]!.variables.topic).toBe("PRODUCTS_DELETE");
    expect(bodies[2]!.variables.webhookSubscription.uri).toBe(
      "https://app.ava.test/api/shopify/webhooks/products/delete",
    );
  ...
  it("strips trailing slash from the callback base URL", async () => {
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(
      jsonResponse(webhookCreateBody({})),
    );
    await registerProductWebhooks(
      "https://example.myshopify.com",
      "shpat_admin",
      "https://app.ava.test/", // trailing slash
      { fetchImpl: fetchMock },
    );
