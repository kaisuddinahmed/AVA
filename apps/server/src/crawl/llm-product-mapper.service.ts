...
// ---------------------------------------------------------------------------
// Config — surfaced via env so ops can adjust without a deploy.
// ---------------------------------------------------------------------------
...
export function getLlmMapperConfig(): LlmMapperConfig {
  return {
...
// ---------------------------------------------------------------------------
// Telemetry — in-memory, per-site running totals.
// ---------------------------------------------------------------------------
...
const telemetry = new Map<string, SiteTelemetry>();

function ensureTelemetry(siteUrl: string): SiteTelemetry {
...
export function getLlmMapperTelemetry(siteUrl: string): SiteTelemetry {
  const t = telemetry.get(siteUrl);
  return t ? { ...t } : { callsMade: 0, successCount: 0, invalidCount: 0, inputTokens: 0, outputTokens: 0 };
}

export function resetLlmMapperTelemetry(siteUrl?: string): void {
  if (siteUrl) telemetry.delete(siteUrl);
  else telemetry.clear();
}

// ---------------------------------------------------------------------------
// LLM client surface — minimal shape so tests can mock easily.
// ---------------------------------------------------------------------------
...
export function extractProductRegion(html: string, maxBytes: number): string {
  if (!html) return "";
  ...
  if (buf.length <= maxBytes) return stripped;
...
export async function extractProductWithLLM(
...
): Promise<GenericProduct | null> {
  const cfg: LlmMapperConfig = { ...getLlmMapperConfig(), ...opts.config };
  ...
  if (!validated.success) {
    tel.invalidCount++;
    log.warn(
      { url, issues: validated.error.issues.slice(0, 3) },
      "[LLM mapper] schema validation failed",
    );
    return null;
  }
...
let groqSingleton: LlmClient | null = null;

function createGroqClient(): LlmClient | null {
  if (groqSingleton) return groqSingleton;
  ...
  groqSingleton = new Groq({ apiKey }) as LlmClient;
