...
import {
  extractProductWithLLM,
  extractProductRegion,
  getLlmMapperTelemetry,
...
function fakeClient(opts: {
  ...
  return {
    chat: {
      completions: {
        create: vi.fn(async () => {
          if (opts.shouldThrow) throw new Error("upstream blew up");
          return {
...
describe("extractProductWithLLM — per-site call cap", () => {
  it("stops calling the client once the per-site cap is reached", async () => {
    ...
    expect(c).toBeNull();
    expect((client.chat.completions.create as ReturnType<typeof vi.fn>).mock.calls).toHaveLength(2);
  });
});
