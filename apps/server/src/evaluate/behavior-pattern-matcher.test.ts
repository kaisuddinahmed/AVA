...
import {
  ...
  type MatcherEvent,
} from "./behavior-pattern-matcher.js";
...
const mockList = BehaviorMappingRepo.listBehaviorMappingsBySite as ReturnType<typeof vi.fn>;

// ── Helpers ───────────────────────────────────────────────────────────────────

function event(eventType: string, extras: Partial<MatcherEvent> = {}): MatcherEvent {
...
describe("detectBehaviorPatterns", () => {
  ...
  it("HIGH_INTENT: detects checkout page type", async () => {
    const result = await detectBehaviorPatterns(
      [event("page_view", { pageType: "checkout" })],
      "site-1"
    );
    const groups = result.map((r) => r.group);
    expect(groups).toContain("HIGH_INTENT");
