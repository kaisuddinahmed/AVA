...
export async function getAddress(req: Request, res: Response): Promise<void> {
  const { visitorKey, siteUrl } = req.query as Record<string, string>;
 ...
 */
export async function saveAddress(req: Request, res: Response): Promise<void> {
  const { visitorKey, siteUrl, addressLine1, addressLine2, city, state, postalCode, country } =
