...
// ---------------------------------------------------------------------------
// Shopify REST product payload (subset — webhook delivers full object)
// ---------------------------------------------------------------------------
...
function verifyWebhookHmac(rawBody: Buffer, hmacHeader: string, apiSecret: string): boolean {
...
function deriveAvailability(p: ShopifyWebhookProduct): "in_stock" | "out_of_stock" | "partial" {
  ...
  if (available === variants.length) return "in_stock";
  return "partial";
}

function priceRange(p: ShopifyWebhookProduct): { min: number | null; max: number | null } {
...
function variantOptions(v: ShopifyWebhookProduct["variants"] extends Array<infer T> | undefined ? T : never): Array<{ name: string; value: string }> {
  const out: Array<{ name: string; value: string }> = [];
  if (v.option1) out.push({ name: "Option1", value: v.option1 });
  if (v.option2) out.push({ name: "Option2", value: v.option2 });
  if (v.option3) out.push({ name: "Option3", value: v.option3 });
  return out;
}

export function toCatalogInput(siteUrl: string, p: ShopifyWebhookProduct): Parameters<typeof SiteCatalogRepo.upsertProduct>[0] {
...
export async function webhookProductsUpdate(req: Request, res: Response) {
  ...
  try {
    ...
    let payload: ShopifyWebhookProduct;
    try {
      payload = JSON.parse(rawBody.toString()) as ShopifyWebhookProduct;
    } catch (err) {
    ...
    log.info(
      { shopDomain, productId: payload.id, handle: payload.handle },
      "[Shopify/products.update] catalog upserted",
    );
  } catch (err) {
