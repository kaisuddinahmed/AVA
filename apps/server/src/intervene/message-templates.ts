...
interface MessageTemplate {
  message: string;
  voiceScript?: string; // ≤80 chars, direct store-associate style
  bubbleText?: string;
  ctaLabel?: string;
  ctaAction?: string;
  uiAdjustments?: Array<{
...
interface TieredTemplate {
  /** Low-signal first visit — gentle, non-intrusive */
  tier1: MessageTemplate;
  /** Returning visitor or showing engagement signals */
  tier2: MessageTemplate;
  /** High intent, repeat dismisser, or high cart value — direct offer */
  tier3: MessageTemplate;
}
...
function selectTier(ctx: SessionContext): 1 | 2 | 3 {
  const highIntent =
    (ctx.isRepeatVisitor && ctx.totalDismissals >= 1) ||
    ctx.cartValue >= 30 ||
    ctx.totalConversions > 0;
  if (highIntent) return 3;

  const hasSignal =
    ctx.isRepeatVisitor ||
    ctx.totalDismissals >= 1 ||
    ctx.totalInterventionsFired >= 3;
  if (hasSignal) return 2;

  return 1;
}

// ── Category templates ────────────────────────────────────────────────────────
const CATEGORY_TEMPLATES: Record<string, TieredTemplate> = {
...
export function getMessageTemplate(
  ...
  if (!tiered) return DEFAULT_TEMPLATE;
