 ...
 */
export async function generateInsightSnapshot(siteUrl) {
    const now = new Date();
    const periodEnd = now;
    ...
    for (const e of frictionEvents) {
        if (e.frictionId)
            frictionCounts[e.frictionId] = (frictionCounts[e.frictionId] ?? 0) + 1;
    }
...
async function generateRecommendations(frictionCtx, totalSessions) {
    if (frictionCtx.length === 0 || !config.groq.apiKey) {
        return frictionCtx.slice(0, 5).map((f) => ({
            ...
            confidence: (f.eventCount >= 100 ? "high" : f.eventCount >= 20 ? "medium" : "low"),
            sampleSize: f.eventCount,
        }));
    }
    const groq = new Groq({ apiKey: config.groq.apiKey });
    const frictionSummary = frictionCtx.map((f) => `- ${f.frictionId} on ${f.page}: ${f.eventCount} detections, ${f.conversions} conversions after AVA intervened`).join("\n");
