...
function detectAction(q) {
    if (/\bcompare\b/i.test(q))
        return 'compare';
    if (/\badd\s+(?:that|it|this|the\s+\w+)\s+to\s+(?:my\s+)?cart\b/i.test(q))
        return 'add_to_cart';
    if (/\badd\s+to\s+(?:my\s+)?cart\b/i.test(q))
        return 'add_to_cart';
    if (/\b(cheaper|less expensive|lower price|more affordable|budget option)\b/i.test(q))
        return 'show_cheaper';
...
function resolveReferences(q) {
    ...
    if (/compare\s+(?:them|both|all)/.test(lq))
        return { referenceIndices: [0, 1] };
    ...
    if (ordM && ORDINALS[ordM[1]] !== undefined)
        return { referenceIndex: ORDINALS[ordM[1]] };
    // "that" / "it" / implicit
    if (/\b(that|it|this)\b/.test(lq))
...
function tryFastParse(q) {
    ...
    if (action === 'compare' && !refs.referenceIndices)
        refs.referenceIndices = [0, 1];
    if (action === 'add_to_cart' && refs.referenceIndex === undefined)
        refs.referenceIndex = 0;
    return { action, attributes: [], ...refs };
}
// ─── Groq slow path ───────────────────────────────────────────────────────────
async function groqParse(q, history) {
    ...
    const system = `You are a shopping intent extraction engine. Return ONLY valid JSON (no markdown):
{
  ...
  "confidence": "high|medium|low"
    ...
    if (ctxLines)
        msgs.push({ role: 'user', content: `Conversation:\n${ctxLines}` });
...
// ─── Public ───────────────────────────────────────────────────────────────────
export async function parseIntent(query, conversationHistory = []) {
    const q = query.trim();
...
export function isShoppingRequest(transcript) {
