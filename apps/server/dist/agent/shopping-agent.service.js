...
function pushHistory(ctx, role, content, products) {
...
// ─── Response generators ──────────────────────────────────────────────────────
async function generateNarration(ctx, pageCtx, intent, products) {
    const productContext = products.slice(0, 3).map((p, i) => `${i + 1}. ${p.title} — $${p.price} ${p.matchedAttributes.length ? `(matches: ${p.matchedAttributes.join(', ')})` : ''}`).join('\n');
    const messages = [
...
async function generateMessage(ctx, pageCtx, userMessage) {
    const messages = [
        { role: 'system', content: buildSystemPrompt(pageCtx) },
        ...ctx.history.slice(-8).map(m => ({ role: m.role, content: m.content })),
...
export async function processQuery(opts) {
    ...
    switch (intent.action) {
        case 'search': {
            ...
            await logAgentAction(sessionId, siteConfig.siteUrl, 'AGENT_SEARCH', intent, products?.map(p => p.id) ?? [], ctx.turnIndex, Date.now() - t0);
            break;
        }
        case 'compare': {
            ...
            if (products.length >= 2) {
                ...
                message = `Comparing ${a.title} and ${b.title}. ${diff}. ${a.matchedAttributes.length ? `The ${a.title} is stronger on: ${a.matchedAttributes.join(', ')}.` : ''}`;
            }
            else {
                message = 'I don\'t have two products to compare yet. Let me search first — what are you looking for?';
            }
            await logAgentAction(sessionId, siteConfig.siteUrl, 'AGENT_COMPARE', intent, products?.map(p => p.id) ?? [], ctx.turnIndex, Date.now() - t0);
            break;
        }
        case 'add_to_cart': {
            ...
            await logAgentAction(sessionId, siteConfig.siteUrl, 'AGENT_CART_CONFIRM', intent, target ? [target.id] : [], ctx.turnIndex, Date.now() - t0);
            break;
        }
        case 'show_cheaper': {
        ...
        case 'navigate': {
            ...
            await logAgentAction(sessionId, siteConfig.siteUrl, 'AGENT_NAVIGATE', intent, [], ctx.turnIndex, Date.now() - t0);
            break;
        }
        case 'clarify': {
            responseType = 'clarification';
            ...
            await logAgentAction(sessionId, siteConfig.siteUrl, 'AGENT_CLARIFY', intent, [], ctx.turnIndex, Date.now() - t0);
            break;
        }
        case 'show_more': {
            if (ctx.lastResults.length > 0) {
                ...
                const lastUserMsg = [...ctx.history].reverse().find(m => m.role === 'user' && m.content !== query && !SHOW_ALL_RE.test(m.content));
                const fallbackQuery = lastUserMsg?.content ?? intent.category ?? null;
                if (fallbackQuery) {
                    const freshIntent = { ...intent, action: 'search', category: fallbackQuery, raw: fallbackQuery };
        ...
        default: {
            ...
            await logAgentAction(sessionId, siteConfig.siteUrl, 'AGENT_FALLBACK', intent, [], ctx.turnIndex, Date.now() - t0);
        }
    }
 ...
 */
export async function handleShoppingQuery(sessionId, transcript, agentCtx) {
 ...
 */
export async function broadcastAgentResponse(sessionId, agentResponse, voicePlayback) {
...
//# sourceMappingURL=shopping-agent.service.js.map