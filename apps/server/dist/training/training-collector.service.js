...
// ============================================================================
import { TrainingDatapointRepo, InterventionRepo, SessionRepo, EventRepo, } from "@ava/db";
 ...
 */
export async function captureTrainingDatapoint(interventionId, outcome) {
    // Only capture terminal outcomes
    if (!TERMINAL_OUTCOMES.has(outcome))
        return null;
    // Skip if already captured (idempotent)
    const existing = await TrainingDatapointRepo.getDatapointByInterventionId(interventionId);
    if (existing)
        return existing.id;
    ...
    if (!intervention || !intervention.evaluation) {
        log.warn(`[TrainingCollector] Intervention ${interventionId} not found or missing evaluation`);
        return null;
    }
    const evaluation = intervention.evaluation;
    const session = await SessionRepo.getSession(intervention.sessionId);
    if (!session) {
        log.warn(`[TrainingCollector] Session ${intervention.sessionId} not found`);
        return null;
    }
    ...
    let pageType = "other";
    try {
        const eventIds = JSON.parse(evaluation.eventBatchIds);
        const events = await EventRepo.getEventsByIds(eventIds);
        rawEventData = JSON.stringify(events.map((e) => ({
            category: e.category,
        ...
        log.warn(`[TrainingCollector] Failed to parse event batch for evaluation ${evaluation.id}`);
    }
    // Calculate session age at evaluation time
    const sessionAgeSec = Math.round((evaluation.timestamp.getTime() - session.startedAt.getTime()) / 1000);
    // Calculate outcome delay
    let outcomeDelayMs;
    const outcomeTimestamp = intervention.dismissedAt ??
