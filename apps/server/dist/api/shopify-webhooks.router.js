...
shopifyWebhooksRouter.post("/uninstall", raw({ type: "*/*" }), shopifyApi.webhookUninstall);
// Mandatory GDPR webhooks (Shopify App Store requirement)
shopifyWebhooksRouter.post("/gdpr/customers/data_request", raw({ type: "*/*" }), shopifyApi.webhookCustomersDataRequest);
shopifyWebhooksRouter.post("/gdpr/customers/redact", raw({ type: "*/*" }), shopifyApi.webhookCustomersRedact);
shopifyWebhooksRouter.post("/gdpr/shop/redact", raw({ type: "*/*" }), shopifyApi.webhookShopRedact);
// Phase 1.3.4 — product lifecycle webhooks
shopifyWebhooksRouter.post("/products/update", raw({ type: "*/*" }), shopifyWebhooks.webhookProductsUpdate);
shopifyWebhooksRouter.post("/products/create", raw({ type: "*/*" }), shopifyWebhooks.webhookProductsUpdate);
shopifyWebhooksRouter.post("/products/delete", raw({ type: "*/*" }), shopifyWebhooks.webhookProductsDelete);
//# sourceMappingURL=shopify-webhooks.router.js.map