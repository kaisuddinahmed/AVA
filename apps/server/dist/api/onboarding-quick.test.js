...
const installGenericMock = SiteConfigRepo.installGenericSite;
const installShopifyMock = SiteConfigRepo.installShopify;
const installWooMock = SiteConfigRepo.installWooCommerce;
function mockRes() {
    let statusCode = 200;
    let body = undefined;
    const r = {
...
beforeEach(() => {
    ...
    SiteCatalogRepo.upsertProduct.mockReset().mockResolvedValue({});
});
...
describe("POST /api/onboarding/quick — Shopify branch", () => {
    it("returns 400 with `requires: [storefrontToken]` when Shopify detected but no token", async () => {
        vi.stubGlobal("fetch", vi.fn(async () => htmlResponse(SHOPIFY_HOME)));
        const res = mockRes();
    ...
    it("forwards to shopifyQuickOnboard when token provided (installShopify gets called)", async () => {
        ...
        vi.stubGlobal("fetch", vi.fn(async (input) => {
            ...
            if (url.includes("/api/2024-10/graphql.json"))
                return jsonResponse(STOREFRONT_BODY);
            if (url.includes("sitemap"))
                return xmlResponse(`<?xml version="1.0"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"></urlset>`);
        ...
        expect(res.getBody().platform).toBe("shopify");
    });
});
...
describe("POST /api/onboarding/quick — WooCommerce branch", () => {
    it("forwards to wooCommerceQuickOnboard (installWooCommerce gets called) even without creds", async () => {
        ...
        expect(res.getBody().platform).toBe("woocommerce");
    });
});
// ── Generic path ───────────────────────────────────────────────────────────
describe("POST /api/onboarding/quick — generic branch", () => {
    it("BFS crawls, runs structured-data ingest, returns coverage + transport='generic_crawl'", async () => {
        vi.stubGlobal("fetch", vi.fn(async (input) => {
            ...
            if (url.includes("/products/"))
                return htmlResponse(GENERIC_PDP("foo", "Foo"));
    ...
    it("LLM fallback is OFF unless LLM_DOM_MAPPER_ENABLED=true in env", async () => {
        ...
        expect(products.bySource.llm).toBe(0);
    });
});
...
//# sourceMappingURL=onboarding-quick.test.js.map