...
function buildOAuthUrl(shop, cfg) {
...
async function exchangeCodeForToken(shop, code, cfg) {
...
async function injectScriptTag(shop, token, widgetSrc) {
...
async function removeScriptTag(shop, token, scriptTagId) {
...
export async function callback(req, res) {
    ...
    if (!verifyShopifyHmac(req.query, cfg.apiSecret)) {
 ...
 */
export async function webhookUninstall(req, res) {
 ...
 */
export function webhookCustomersDataRequest(req, res) {
    const rawBody = req.body;
    const hmacHeader = req.headers["x-shopify-hmac-sha256"] ?? "";
    const cfg = getShopifyConfig();
    if (!verifyWebhookHmac(rawBody, hmacHeader, cfg.apiSecret)) {
 ...
 */
export function webhookCustomersRedact(req, res) {
    const rawBody = req.body;
    const hmacHeader = req.headers["x-shopify-hmac-sha256"] ?? "";
    const cfg = getShopifyConfig();
    if (!verifyWebhookHmac(rawBody, hmacHeader, cfg.apiSecret)) {
 ...
 */
export async function webhookShopRedact(req, res) {
    const rawBody = req.body;
    const hmacHeader = req.headers["x-shopify-hmac-sha256"] ?? "";
    const cfg = getShopifyConfig();
    if (!verifyWebhookHmac(rawBody, hmacHeader, cfg.apiSecret)) {
