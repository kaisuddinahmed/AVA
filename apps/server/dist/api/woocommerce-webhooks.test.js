...
import { webhookProductsUpdate, webhookProductsDelete, toCatalogInput, verifyWooSignature, } from "./woocommerce-webhooks.api.js";
...
const asRes = (m) => m;
function signedHeaders(rawBody, source, topic = "product.updated") {
...
describe("webhookProductsUpdate — HMAC + upsert", () => {
    ...
    it("normalizes the source URL — path is stripped before SiteConfig lookup", async () => {
        const body = Buffer.from(JSON.stringify(productPayload()));
        const req = asReq(body, signedHeaders(body, "https://shop.example/some/path/?with=qs"));
        const res = mockRes();
        await webhookProductsUpdate(req, asRes(res));
...
//# sourceMappingURL=woocommerce-webhooks.test.js.map