...
export async function listJobRuns(req, res) {
    try {
        const { jobName, status, limit = "50", offset = "0", } = req.query;
    ...
    try {
        ...
        if (job === "nightly_batch") {
            ...
            try {
                ...
                await JobRunRepo.completeJobRun(jobRun.id, result, Date.now() - startTime);
            ...
            try {
                ...
                await JobRunRepo.completeJobRun(jobRun.id, { checked: true }, Date.now() - startTime);
 ...
 */
export async function getNextRun(_req, res) {
    try {
        const runner = getJobRunner();
