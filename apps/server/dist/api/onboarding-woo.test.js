...
const upsertProductMock = SiteCatalogRepo.upsertProduct;
const upsertSiteMapMock = SiteMapRepo.upsertSiteMap;
function mockRes() {
    let statusCode = 200;
    let body = undefined;
    const r = {
...
describe("POST /api/onboarding/woocommerce-quick — REST v3 path", () => {
    ...
    it("propagates Woo 'unauthorized' as 401 so wizard can re-prompt for creds", async () => {
        ...
        expect(res.getBody().kind).toBe("unauthorized");
    });
});
// ── Detection failures ──────────────────────────────────────────────────────
describe("POST /api/onboarding/woocommerce-quick — detection guards", () => {
    it("400s when the URL is not a WooCommerce store", async () => {
        ...
        expect(res.getStatus()).toBe(400);
        expect(res.getBody().error).toMatch(/not appear to be a WooCommerce/);
