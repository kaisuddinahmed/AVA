 ...
 */
export async function getLatestInsights(req, res) {
    try {
        ...
        let recommendations = [];
        try {
            recommendations = JSON.parse(snapshot.recommendations);
        }
        catch { /* ignore */ }
        let topFrictionTypes = [];
        try {
            topFrictionTypes = JSON.parse(snapshot.topFrictionTypes);
        }
        catch { /* ignore */ }
        res.json({
            siteUrl,
 ...
 */
export async function getCROFindings(req, res) {
    try {
        const siteUrl = parseSiteUrl(req);
        if (!siteUrl) {
            res.status(400).json({ error: "siteUrl is required" });
        ...
        let findings = [];
        try {
            findings = JSON.parse(snapshot.croFindings);
        }
        catch { /* ignore */ }
...
//# sourceMappingURL=insights.api.js.map