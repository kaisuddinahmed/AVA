...
async function fetchProducts(shop, token) {
    try {
        const resp = await fetch(`https://${shop}/admin/api/2024-01/products.json?limit=5&fields=id,title,variants`, { headers: { "X-Shopify-Access-Token": token } });
        if (!resp.ok)
            return [];
...
async function fetchActiveTheme(shop, token) {
    try {
        const resp = await fetch(`https://${shop}/admin/api/2024-01/themes.json?role=main`, { headers: { "X-Shopify-Access-Token": token } });
        if (!resp.ok)
            return null;
        const data = await resp.json();
 ...
 */
function buildShopifySelectorMappings() {
    return [
...
function buildShopifyFrictionMappings() {
    return [
        {
 ...
 */
export async function seedShopifyMappings(shop, accessToken) {
    ...
    log.info({ shop, productCount: products.length, theme: theme?.name ?? "unknown" }, "Shopify store metadata fetched");
...
//# sourceMappingURL=shopify-mapper.service.js.map