 ...
 */
export async function emitSessionExitWebhook(sessionId) {
    const session = await SessionRepo.getSession(sessionId);
    if (!session)
        return;
    // Skip if session had a conversion
    if (session.totalConversions > 0)
        return;
    // Get site config
...
// Internal helpers
// ---------------------------------------------------------------------------
async function buildPayload(session, sessionId) {
...
async function sendWithRetry(deliveryId, url, secret, payload, attempt = 1) {
