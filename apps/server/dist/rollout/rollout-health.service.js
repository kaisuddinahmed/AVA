...
import { promoteStage, rollbackRollout, } from "./rollout.service.js";
 ...
 */
export async function evaluateRolloutHealth(rolloutId) {
    ...
    if (healthCriteria.minConversionRate != null) {
        const passed = treatment.conversionRate >= healthCriteria.minConversionRate;
        checks.push({
        ...
        if (treatment.sampleSize >= (healthCriteria.minSampleSize ?? 10) &&
    ...
    if (healthCriteria.maxDismissalRate != null) {
        const passed = treatment.dismissalRate <= healthCriteria.maxDismissalRate;
        checks.push({
    ...
    const hasInsufficientData = checks.some((c) => c.name === "min_sample_size" && !c.passed);
    let status;
    let recommendation;
 ...
 */
export async function checkAllRolloutsHealth() {
        ...
        try {
            ...
            if (health.recommendation === "rollback") {
                const failedChecks = health.checks
                    .filter((c) => !c.passed)
                    .map((c) => c.name);
                await rollbackRollout(rollout.id, `Auto-rollback: ${failedChecks.join(", ")}`);
                log.info(`[RolloutHealth] Auto-rolled back rollout ${rollout.name}: ${failedChecks.join(", ")}`);
            }
                ...
                if (currentStage) {
                    ...
                    if (hoursInStage >= currentStage.durationHours) {
                        ...
                        log.info(`[RolloutHealth] Auto-promoted rollout ${rollout.name} to stage ${rollout.currentStage + 1}`);
                    }
                }
            }
        }
        catch (error) {
            log.error(`[RolloutHealth] Failed to check rollout ${rollout.id}:`, error);
        }
    }
}
// ---------------------------------------------------------------------------
...
//# sourceMappingURL=rollout-health.service.js.map