...
// ---------------------------------------------------------------------------
async function getNetworkPriorSeverity(frictionId) {
    try {
 ...
 */
export async function runFastEvaluation(input) {
    const { sessionCtx, detectedFrictionIds, pageType, eventCount, latestScrollDepthPct = 0, latestSessionSequence = 0, priorExitIntentCount = 0, siteTotalSessions = 9999 } = input;
    // ── 1. Synthesize intent hint ──────────────────────────────────────────
    let syntheticIntent = SYNTHETIC_INTENT_BASE[pageType] ?? 10;
    if (sessionCtx.isLoggedIn)
        syntheticIntent += 5;
    if (sessionCtx.isRepeatVisitor)
        syntheticIntent += 5;
    if (sessionCtx.cartItemCount > 0)
        syntheticIntent += 8;
    ...
    if (detectedFrictionIds.length > 0) {
        if (USE_NETWORK_PRIORS) {
            ...
            const priorSeverities = await Promise.all(detectedFrictionIds.map(async (id) => {
    ...
    if (eventCount >= 5)
        syntheticClarity += 10;
    if (sessionCtx.sessionAgeSec > 120)
        syntheticClarity += 10;
    ...
    if (sessionCtx.cartValue > 50)
        syntheticValue = 35;
    if (sessionCtx.cartValue > 100)
        syntheticValue = 50;
    if (sessionCtx.cartValue > 200)
        syntheticValue = 65;
    if (sessionCtx.isLoggedIn)
        syntheticValue += 8;
    if (sessionCtx.isRepeatVisitor)
 ...
 */
export function shouldEscalateToLLM(fastResult) {
    ...
    if (mswimResult.composite_score >= 65)
        return true;
    ...
    if (mswimResult.gate_override?.startsWith("FORCE_ESCALATE"))
        return true;
...
export function computeAbandonmentScore(input) {
    ...
    if (input.pageType === "pdp" && input.scrollDepthPct < 30)
        score += 8;
    // Minimal events = low engagement
    if (input.sessionSequence < 3)
        score += 10;
    else if (input.sessionSequence < 6)
        score += 5;
    ...
    if (input.compositeScore >= 50)
        score += 5;
    return clamp(score);
}
function buildFastNarrative(ctx, frictionIds, result) {
    const parts = [];
    // Page context
    parts.push(`Visitor on ${ctx.pageType} page.`);
    // Session state
    if (ctx.isLoggedIn)
        parts.push("Logged-in user.");
    if (ctx.isRepeatVisitor)
        parts.push("Returning visitor.");
    if (ctx.cartValue > 0)
        parts.push(`Cart: $${ctx.cartValue.toFixed(2)}.`);
    // Behavior groups
...
function buildFastReasoning(result, frictionIds) {
    const parts = [];
    parts.push(`[fast-engine] Composite=${result.composite_score.toFixed(1)} → ${result.tier}.`);
    parts.push(`Signals: I=${result.signals.intent} F=${result.signals.friction} C=${result.signals.clarity} R=${result.signals.receptivity} V=${result.signals.value}.`);
    if (frictionIds.length > 0) {
 ...
 */
export function inferFrictionFromContext(ctx) {
    // Specific gate-flag frictions (highest priority — these are definitive)
    if (ctx.hasPaymentFailure)
        return "F096"; // payment failure
    if (ctx.hasTechnicalError)
        return "F161"; // technical/JS error
    if (ctx.hasOutOfStock)
        return "F053"; // out of stock
    if (ctx.hasShippingIssue)
        return "F236"; // shipping concern
    if (ctx.hasCheckoutTimeout)
        return "F112"; // checkout timeout
    if (ctx.hasHelpSearch)
        return "F036"; // searched for help
...
//# sourceMappingURL=fast-evaluator.js.map