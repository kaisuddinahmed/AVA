...
import { BEHAVIOR_GROUP_DEFINITIONS, BEHAVIOR_GROUP_PRIORITY, } from "@ava/shared";
 ...
 */
function detectActiveGroups(events) {
    ...
    // ── HIGH_INTENT ───────────────────────────────────────────────────────────
    {
        ...
        if (eventTypes.includes("add_to_cart"))
            evidence.push("add_to_cart");
        if (eventTypes.includes("quick_add"))
            evidence.push("quick_add");
        if (pageTypes.includes("checkout"))
            evidence.push("checkout_page");
        if (pageTypes.includes("cart") && eventTypes.includes("cart_view"))
            evidence.push("cart_view");
        if (evidence.length >= 1)
            active.set("HIGH_INTENT", evidence);
    }
    // ── COMPARISON ────────────────────────────────────────────────────────────
    {
        const evidence = [];
        ...
        if (pdvCount >= 3)
            evidence.push("product_detail_view_x3");
        if (eventTypes.includes("size_guide_open"))
            evidence.push("size_guide_open");
        if (eventTypes.includes("description_toggle"))
            evidence.push("description_toggle");
        if (eventTypes.includes("size_select"))
            evidence.push("size_select");
        if (eventTypes.includes("color_select"))
            evidence.push("color_select");
        if (eventTypes.includes("filter_applied"))
            evidence.push("filter_applied");
        if (eventTypes.includes("sort_applied"))
            evidence.push("sort_applied");
        if (evidence.length >= 2)
            active.set("COMPARISON", evidence);
    }
    // ── HESITATION ────────────────────────────────────────────────────────────
    {
        const evidence = [];
        const hasVariantSelect = eventTypes.some((t) => ["size_select", "color_select"].includes(t));
        ...
        if (evidence.length >= 1)
            active.set("HESITATION", evidence);
    }
    // ── DISCOVERY ─────────────────────────────────────────────────────────────
    {
        const evidence = [];
        ...
        const isFirstVisit = events.some((e) => e.rawSignals?.is_first_visit === true ||
            e.rawSignals?.session_sequence_number === 1);
    ...
    {
        const evidence = [];
        if (eventTypes.includes("exit_intent"))
            evidence.push("exit_intent");
        if (eventTypes.includes("rage_click"))
            evidence.push("rage_click");
        if (eventTypes.includes("dead_click"))
            evidence.push("dead_click");
        if (eventTypes.includes("tab_switch"))
            evidence.push("tab_switch");
        // Idle friction codes
        if (events.some((e) => e.frictionId && ["F068", "F069", "F400"].includes(e.frictionId)))
 ...
 */
async function resolvePatterns(activeGroups, siteConfigId) {
    ...
    try {
        ...
        mappings = raw.filter((m) => m.isActive);
    }
    catch {
    ...
    for (const group of BEHAVIOR_GROUP_PRIORITY) {
        ...
        if (mappings.length > 0) {
            ...
            for (const mapping of mappings) {
                if (!def.patternIds.includes(mapping.patternId))
                    continue;
                if (seen.has(mapping.patternId))
                    continue;
 ...
 */
export async function detectBehaviorPatterns(events, siteConfigId) {
 ...
 */
export function extractActiveGroups(patterns) {
 ...
 */
export function computeBehaviorBoosts(groups) {
    let intentBoost = 0;
    let clarityBoost = 0;
...
//# sourceMappingURL=behavior-pattern-matcher.js.map