 ...
 */
export async function resolveExperimentOverrides(sessionId, siteUrl) {
    // Check feature flag
    ...
    try {
        ...
        let assignment = await ExperimentRepo.getSessionAssignment(experiment.id, sessionId);
        if (!assignment) {
            // Deterministically assign
            const { enrolled, variantId } = assignVariant(sessionId, experiment.id, variants, experiment.trafficPercent);
            if (!enrolled || !variantId)
                return null;
            // Persist assignment
            assignment = await ExperimentRepo.assignSession(experiment.id, sessionId, variantId);
        }
