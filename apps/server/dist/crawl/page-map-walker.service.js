...
// ---------------------------------------------------------------------------
async function fetchSitemapText(url, fetchImpl) {
    try {
...
async function collectUrls(rootSitemap, fetchImpl, maxUrls) {
    const collected = [];
...
function pushFrom(parsed, depth, queue, collected, maxUrls) {
 ...
 */
function derivePatternFromSamples(samples) {
    if (samples.length === 0)
        return "/";
    let firstPath;
    try {
...
function patternFor(pageType, samples) {
 ...
 */
export async function walkPageMap(input) {
    ...
    for (const url of urls) {
        ...
        if (list.length < MAX_SAMPLES_KEPT)
            list.push(url);
...
//# sourceMappingURL=page-map-walker.service.js.map