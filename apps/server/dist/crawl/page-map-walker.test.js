...
const upsertMock = SiteMapRepo.upsertSiteMap;
...
describe("walkPageMap — Shopify sitemap fixture", () => {
    it("classifies + groups URLs and persists one SiteMap row per pageType", async () => {
        const fetchMock = vi.fn().mockResolvedValue(xmlResponse(fixture("sitemap-shopify.xml")));
...
describe("walkPageMap — confidence", () => {
    it("scales confidence by URL count (log-shaped)", async () => {
        const manyPdps = (n) => `<?xml version="1.0"?><urlset>${Array.from({ length: n }, (_, i) => `<url><loc>https://x.test/products/p${i}</loc></url>`).join("")}</urlset>`;
        // 1 URL → ~0.75
        upsertMock.mockReset();
        const fetchMock1 = vi.fn().mockResolvedValue(xmlResponse(manyPdps(1)));
        ...
        upsertMock.mockReset();
        const fetchMock2 = vi.fn().mockResolvedValue(xmlResponse(manyPdps(100)));
...
describe("walkPageMap — persistence", () => {
    ...
    it("caps samples at 5 even when many URLs match", async () => {
        const manyPdps = `<urlset>${Array.from({ length: 20 }, (_, i) => `<url><loc>https://x.test/products/p${i}</loc></url>`).join("")}</urlset>`;
        const fetchMock = vi.fn().mockResolvedValue(xmlResponse(manyPdps));
...
describe("walkPageMap — failure modes", () => {
    ...
    it("tolerates a malformed sitemap body", async () => {
        const fetchMock = vi.fn().mockResolvedValue(xmlResponse("not xml"));
        const result = await walkPageMap({
            siteUrl: "https://x.test",
            sitemapUrl: "https://x.test/sm.xml",
            fetchImpl: fetchMock,
...
//# sourceMappingURL=page-map-walker.test.js.map