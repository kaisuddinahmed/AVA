...
import { toGenericCatalogInput, } from "./catalog-ingest.service.js";
describe("toGenericCatalogInput — honest availability persistence", () => {
    ...
    it("preserves explicit availability values without modification", () => {
        const base = {
            ...
            currency: "USD", url: "https://x", sourceSignal: "jsonld",
        };
...
import { ingestGenericCatalog } from "./catalog-ingest.service.js";
function pdpJsonLdPage(url, name, price) {
    return {
        url,
...
import { resetLlmMapperTelemetry, } from "./llm-product-mapper.service.js";
function llmClientReturning(payload) {
    return {
        chat: {
            completions: {
                create: vi.fn(async () => ({
...
describe("ingestGenericCatalog — LLM fallback integration", () => {
    ...
    it("falls back to the LLM mapper only when structured data is missing AND fallback is enabled", async () => {
        ...
        expect(llmClient.chat.completions.create.mock.calls).toHaveLength(1);
    });
