...
import { compareFingerprints, checkDriftForSite, } from "./selector-drift.service.js";
const listForSiteMock = SiteSelectorFingerprintRepo.listForSite;
const recordDriftMock = SiteSelectorFingerprintRepo.recordDrift;
const hasRecentAlertMock = DriftAlertRepo.hasRecentAlert;
const createAlertMock = DriftAlertRepo.createAlert;
beforeEach(() => {
...
describe("compareFingerprints", () => {
    it("score=1 when fingerprint hashes match exactly", () => {
        const r = compareFingerprints({ hash: "h1", selectors: { addToCart: ".btn-x" } }, { hash: "h1", selectors: { addToCart: ".btn-z" } });
        expect(r.score).toBe(1);
        expect(r.identicalHash).toBe(true);
    });
    it("score=1 when both hash AND selectors match exactly", () => {
        const r = compareFingerprints({ hash: "h1", selectors: { addToCart: ".btn-x", price: ".price" } }, { hash: "h1", selectors: { addToCart: ".btn-x", price: ".price" } });
        expect(r.score).toBe(1);
    });
    it("identifies missing / changed / added keys when hashes differ", () => {
        const r = compareFingerprints({ hash: "h1", selectors: { addToCart: ".btn", price: ".old-price", cartCount: ".badge" } }, { hash: "h2", selectors: { addToCart: ".btn", price: ".new-price", quickView: ".q" } });
    ...
    it("score=0 when no shared keys agree", () => {
        const r = compareFingerprints({ hash: "h1", selectors: { a: ".x" } }, { hash: "h2", selectors: { a: ".y" } });
        // 0 agreed / 1 union = 0
...
//# sourceMappingURL=selector-drift.test.js.map