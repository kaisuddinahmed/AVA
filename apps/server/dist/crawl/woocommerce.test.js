...
import { listProducts, listAllProducts, } from "./woocommerce.client.js";
...
// ── Fixture builders ────────────────────────────────────────────────────────
function storeApiProduct(overrides = {}) {
    return {
        id: overrides.id ?? 101,
...
function restV3Product(overrides = {}) {
    return {
        id: overrides.id ?? 501,
        name: "Raw Linen Tee",
        slug: overrides.slug ?? "raw-linen-tee",
...
describe("WooCommerce.listProducts — Store API (public)", () => {
    it("requests /wp-json/wc/store/v1/products with no auth header", async () => {
        const fetchMock = vi.fn().mockResolvedValue(jsonResponse([storeApiProduct()]));
...
describe("WooCommerce.listProducts — REST v3 (authenticated)", () => {
    it("sends Basic auth derived from consumer key/secret", async () => {
        const fetchMock = vi.fn().mockResolvedValue(jsonResponse([restV3Product()]));
...
describe("WooCommerce.listAllProducts — pagination drain", () => {
    it("uses X-WP-TotalPages header to drive nextPage cursor", async () => {
        const fetchMock = vi.fn()
            .mockResolvedValueOnce(jsonResponse([storeApiProduct({ id: 1 }), storeApiProduct({ id: 2 })], 200, { "x-wp-totalpages": "2", "x-wp-total": "3" }))
            .mockResolvedValueOnce(jsonResponse([storeApiProduct({ id: 3 })], 200, { "x-wp-totalpages": "2", "x-wp-total": "3" }));
    ...
    it("respects maxProducts cap", async () => {
        const fetchMock = vi.fn().mockResolvedValue(jsonResponse([storeApiProduct({ id: 1 }), storeApiProduct({ id: 2 }), storeApiProduct({ id: 3 })], 200, { "x-wp-totalpages": "10" }));
    ...
    it("falls back to page-fullness heuristic when X-WP headers are absent", async () => {
        const fetchMock = vi.fn()
            .mockResolvedValueOnce(jsonResponse(Array.from({ length: 50 }, (_, i) => storeApiProduct({ id: i + 1 }))))
...
describe("WooCommerce — error taxonomy", () => {
    ...
    it("non-array body → malformed_response", async () => {
        const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ message: "nope" }));
        await expect(listProducts("https://shop.example", STORE_CREDS, 1, { fetchImpl: fetchMock }))
