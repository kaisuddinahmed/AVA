...
// Internals
// ---------------------------------------------------------------------------
function buildUrl(siteUrl, credentials, opts) {
    let base;
    try {
 ...
 */
function parseAmount(raw, minorUnit) {
    if (raw == null || raw === "")
        return null;
    const n = Number(raw);
...
function normalizeStoreApiProduct(raw) {
...
function normalizeRestV3Product(raw) {
    ...
    const tags = (raw.tags ?? []).map((t) => t.name ?? "").filter(Boolean);
    const available = raw.status === "publish" &&
...
// Request plumbing
// ---------------------------------------------------------------------------
async function fetchProductsPage(siteUrl, credentials, page, pageSize, fetchImpl, apiPath) {
    ...
    if (resp.status === 401)
        throw new WooCommerceError("unauthorized", "Bad consumer key/secret", { status: 401 });
    if (resp.status === 403)
        throw new WooCommerceError("forbidden", "WooCommerce REST API forbidden", { status: 403 });
    if (resp.status === 404)
        throw new WooCommerceError("not_found", "WooCommerce endpoint not found — site may not run Woo", { status: 404 });
    if (resp.status === 429) {
 ...
 */
export async function listProducts(siteUrl, credentials, page = 1, opts) {
    ...
    const { raw, totalPages, total } = await fetchProductsPage(siteUrl, credentials, page, pageSize, fetchImpl, opts?.apiPath);
    const normalize = credentials.kind === "store_api"
        ? (r) => normalizeStoreApiProduct(r)
        : (r) => normalizeRestV3Product(r);
 ...
 */
export async function listAllProducts(siteUrl, credentials, opts) {
