...
export function extractMicrodata(html) {
    ...
    while ((m = tagRe.exec(html)) !== null) {
        if (m[1]) {
            ...
            const frame = { tag, attrs, textStart: m.index + m[0].length };
            // Start a new item?
            ...
            if (attrs["itemprop"] && parentItem) {
                ...
                if (!frame.item && isSelfClosing) {
                    ...
                    if (v != null)
                        setProperty(parentItem, frame.itemprop, v);
                }
            }
            if (!isSelfClosing)
                stack.push(frame);
                ...
                else if (frame.itemprop)
                    setProperty(parentItem, frame.itemprop, frame.item);
            }
            continue;
        }
        // Closing tag
        ...
        for (let i = stack.length - 1; i >= 0; i--) {
            if (stack[i].tag !== closeTag)
                continue;
            const frame = stack[i];
            const closedRange = [frame.textStart, m.index];
            stack.splice(i, 1);
            if (frame.item) {
                // Emit item
                ...
                if (text)
                    setProperty(frame.parent, frame.itemprop, text);
            }
            break;
        }
    }
...
function typeFromItemtype(itemtype) {
    if (!itemtype)
        return "Thing";
...
function valueFromAttrs(tag, attrs) {
    // meta uses content, anchors/img/links use their resource attribute.
    if (tag === "meta" && attrs["content"] != null)
        return attrs["content"];
    if ((tag === "a" || tag === "link") && attrs["href"] != null)
        return attrs["href"];
    if (tag === "img" && attrs["src"] != null)
        return attrs["src"];
    if ((tag === "time" || tag === "data") && attrs["datetime"] != null)
        return attrs["datetime"];
    if (tag === "data" && attrs["value"] != null)
        return attrs["value"];
    return null;
}
...
function setProperty(item, key, value) {
...
//# sourceMappingURL=structured-data.extractor.js.map