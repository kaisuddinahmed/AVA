...
import { listProducts, listAllProducts, ShopifyStorefrontError, } from "./shopify-storefront.client.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
describe("listProducts — request shape", () => {
    ...
    it("accepts a bare hostname (no protocol) and builds the right URL", async () => {
        const fetchMock = vi.fn().mockResolvedValue(jsonResponse(listProductsBody({ products: [] })));
...
describe("listProducts — response transform", () => {
    it("flattens edges → products and parses prices to numbers", async () => {
        const fetchMock = vi.fn().mockResolvedValue(jsonResponse(listProductsBody({
...
describe("ShopifyStorefrontError — kinds", () => {
    ...
    it("maps GraphQL errors[] → graphql kind", async () => {
        const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ errors: [{ message: "Field 'foo' doesn't exist" }] }));
        await expect(listProducts("x.myshopify.com", "tok", null, { fetchImpl: fetchMock }))
    ...
    it("maps malformed JSON body → malformed_response", async () => {
        const fetchMock = vi.fn().mockResolvedValue(new Response("not json", { status: 200, headers: { "content-type": "text/html" } }));
        await expect(listProducts("x.myshopify.com", "tok", null, { fetchImpl: fetchMock }))
            .rejects.toMatchObject({ kind: "malformed_response" });
    });
});
//# sourceMappingURL=shopify-storefront.test.js.map