...
const DEEPGRAM_STT_URL = "https://api.deepgram.com/v1/listen?model=nova-2&smart_format=true&language=en";
/**
 * POST /api/voice/tts
 * Body (JSON): { text: string, model?: string }
 * Response: audio/mpeg binary (pipe from Deepgram)
 */
export async function ttsProxy(req, res) {
    ...
    try {
        const upstream = await fetch(`${DEEPGRAM_TTS_URL}?model=${encodeURIComponent(voiceModel)}&encoding=mp3`, {
            method: "POST",
            headers: {
 ...
 */
export async function sstProxy(req, res) {
    if (!config.voice.deepgramApiKey) {
    ...
    const contentType = req.headers["x-audio-content-type"] ||
...
//# sourceMappingURL=voice-proxy.api.js.map