...
function appendToHistory(sessionId, userTurn, assistantTurn) {
...
function buildSystemPrompt(pageCtx) {
    let prompt = "You are AVA, a friendly personal shopping assistant embedded on an e-commerce site. " +
 ...
 */
export async function handleVoiceQuery(ws, sessionId, transcript, pageCtx) {
    // 1. Global toggle
    ...
    let answer = "Great question! Let me help you find exactly what you need.";
    ...
    log.info(`[VoiceResponder] Replied to session ${sessionId} (turn ${turnCount}): "${answer.slice(0, 60)}…"` +
        ` (voice=${voicePlayback})`);
...
//# sourceMappingURL=voice-responder.service.js.map