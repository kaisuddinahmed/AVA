...
var AVA = (() => {
  ...
  var AVAWidget = class {
    ...
    // Voice (TTS + ASR)
    voiceManager = null;
    _welcomeSpoken = false;   // tracks whether welcome TTS has fired this session
    ...
    mount() {
      ...
      if (!this.isMobile) {
        ...
        _face.style.cssText = "width:100%;height:100%;display:block;object-fit:contain;cursor:pointer;user-select:none;-webkit-user-select:none;";
        _iw.appendChild(_face);
        // Hold-to-record: must hold ≥350ms before mic activates — quick tap does nothing
        let _facePressing = false;
        let _faceRecording = false;
        let _facePressTimer = null;
        const _startFaceMic = (e) => {
          e.stopPropagation();
          if (!this.speechRecognizer) return;
          _facePressing = true;
          if (e.pointerId !== undefined) _face.setPointerCapture(e.pointerId);
          // Only activate mic after 350ms hold — prevents single-tap from triggering
          _facePressTimer = setTimeout(() => {
            if (!_facePressing) return;
            if (this.speechRecognizer.isRecording || this.speechRecognizer.isProcessing) return;
            _faceRecording = true;
            _ring.style.cssText = "position:absolute;top:50%;left:50%;width:60px;height:60px;margin:-30px 0 0 -30px;border-radius:50%;pointer-events:none;display:block;border:2.5px solid #e94560;animation:ava-face-pulse 0.8s ease-in-out infinite;";
            this._faceMicActive = true;
            this.speechRecognizer.start().catch((err) => {
              _ring.style.display = "none";
              _faceRecording = false;
              _facePressing = false;
              this._faceMicActive = false;
              console.warn("[AVA] Face mic start failed:", err);
            });
          }, 350);
        };
        const _stopFaceMic = (e) => {
          _facePressing = false;
          clearTimeout(_facePressTimer);
          _facePressTimer = null;
          if (_faceRecording) {
            _faceRecording = false;
            _ring.style.display = "none";
            if (this.speechRecognizer?.isRecording) this.speechRecognizer.stop();
          }
          // Quick tap (no recording started) → open chat
          else if (this.state === "minimized") {
            this.handleToggleClick();
          }
        };
        _face.addEventListener("pointerdown", _startFaceMic);
        _face.addEventListener("pointerup", _stopFaceMic);
        _face.addEventListener("pointercancel", _stopFaceMic);
        ...
        _badge.addEventListener("click", (e) => {
          e.stopPropagation();
          if (this.voiceManager) this.voiceManager._muted = false;
        ...
        for(let i=0;i<3;i++){const d=document.createElement("div");d.style.cssText="width:5px;height:5px;border-radius:50%;background:rgba(99,102,241,0.9);";_capsule.appendChild(d);}
        ...
        this._missedVoiceCount = 0;
        this._lastVoiceScript = null;
        if (this.voiceManager) {
          const _vm = this.voiceManager;
          const _origPlay = _vm._playSpeech.bind(_vm);
          _vm._playSpeech = async (script) => {
            this._lastVoiceScript = script;
            if (this.voiceManager?.isMuted) { this._missedVoiceCount++; this.renderToggle(); return; }
            this._isTalking = true;
            this._audioStarting = true;
            this.renderToggle();
            await _origPlay(script);
            this._audioStarting = false;
            // _origPlay resolves when audio STARTS (not ends) — hook ended event
            if (_vm.currentAudio) {
              _vm.currentAudio.addEventListener("ended", () => {
                this._isTalking = false;
                this.renderToggle();
              }, { once: true });
              _vm.currentAudio.addEventListener("error", () => {
                this._isTalking = false;
                this.renderToggle();
              }, { once: true });
            } else {
          ...
          const _origStop = _vm.stopCurrent.bind(_vm);
          _vm.stopCurrent = () => {
            _origStop();
            if (!this._audioStarting) {
              this._isTalking = false;
              this.renderToggle();
            }
          };
      ...
      const _welcomeMsg = "Hi, I am AVA. I am here to assist you with your shopping today. Just let me know if you need any assistance.";
      const _fireWelcome = () => {
        ...
        if (!this.messages.some(m => m.id === "ava_welcome")) {
          this.messages.push({
            ...
            type: "assistant",
            content: _welcomeMsg,
            payload: { action_code: "WELCOME", intervention_id: "ava_welcome", voice_script: _welcomeMsg },
            timestamp: Date.now()
          });
        }
        this.hasUnread = true;
        this.renderToggle();
        // VoiceManager's activate listener (registered before this one) has already
        // set _userActivated=true for this same click event — call speak() directly.
        // Guard with _welcomeSpoken so handleToggleClick doesn't double-play.
        if (!this._welcomeSpoken && this.config.voiceAutoPlay !== false) {
          this._welcomeSpoken = true;
          this.voiceManager?.speak(_welcomeMsg).catch(() => {});
        }
      };
    ...
    handleIntervention(payload) {
      switch (payload.type) {
        case "passive":
          ...
          break;
        case "nudge":
          if (this._faceWidget && !this._userHasSpoken && payload.intervention_id !== "ava_welcome") {
            payload = { ...payload, voice_enabled: false, voice_script: void 0 };
          }
          this.currentNudge = payload;
          this.hasUnread = true;
          ...
          // Welcome TTS is deferred to handleToggleClick so audio and bubble appear together
          if (payload.voice_enabled && payload.voice_script && this.config.voiceAutoPlay !== false && payload.intervention_id !== "ava_welcome") {
            this.voiceManager?.speak(payload.voice_script).catch(() => {
            });
          }
          break;
        case "active":
          // Suppress TTS when: user typed (not spoke), OR server-pushed intervention
          if (payload.action_code === "VOICE_REPLY" || payload.action_code === "AGENT_VOICE") {
            // User-initiated reply — suppress voice if last input was text
            if (!this._lastInputWasVoice) {
              payload = { ...payload, voice_enabled: false, voice_script: void 0 };
            }
          } else if (this._faceWidget && !this._userHasSpoken) {
            // Server-pushed intervention — suppress until user has spoken
            payload = { ...payload, voice_enabled: false, voice_script: void 0 };
          }
          this.currentNudge = null;
          if (this.signalCollapseTimeout) clearTimeout(this.signalCollapseTimeout);
          // Face-mic mode: keep panel closed — log silently, play TTS only
          if ((payload.action_code === "VOICE_REPLY" || payload.action_code === "AGENT_VOICE") && this.state === "minimized") {
            this._faceMicActive = false;
            setTimeout(() => {
              this.messages.push({
              ...
              if (payload.action_code === "AGENT_VOICE") {
                const avaSearch = window.__AVA_SEARCH__;
                if (typeof avaSearch === "function") {
                  const q = payload.searchQuery || (payload.navigateTo ? new URL(payload.navigateTo, location.href).searchParams.get("q") : null);
                  if (q) avaSearch(q);
                }
              }
            }, 300);
          ...
          setTimeout(() => {
            ...
            // Auto-filter store when agent returns a search query (AGENT_VOICE only)
            if (payload.action_code === "AGENT_VOICE") {
              const avaSearch = window.__AVA_SEARCH__;
              if (typeof avaSearch === "function") {
                const q = payload.searchQuery || (payload.navigateTo ? new URL(payload.navigateTo, location.href).searchParams.get("q") : null);
    ...
    renderToggle() {
      if (this._faceWidget && this._faceRing) {
        ...
        if (this._faceCapsule) this._faceCapsule.title = this.state === "expanded" ? "Close chat" : "Open chat";
        return;
      }
      updateToggleButton(this.toggleBtn, this.state, this.hasUnread, this.config, this.currentNudge ? deriveLabelText(this.currentNudge) : void 0);
    }
    renderNudge() {
    ...
    // ─── Dynamic section renderer — called on every render ──────────────────
    // Fills lead area, messages, and footer in place. The header is never
    // touched after first build, which eliminates the per-message blink.
    _fillDynamicSections(leadArea, contentEl, footerEl) {
      // ── Lead area ────────────────────────────────────────────────────────
      leadArea.innerHTML = "";
      const hasUserMsg = this.messages.some(m => m.type === "user");
      if (!hasUserMsg) {
        if (this.isTyping) {
      ...
      if (!hasSupporting && !this.isTyping) {
        ...
        for (const msg of this.messages) {
          ...
          const wrapper = this.el("div", { style: `display:flex;flex-direction:column;align-items:${isUserMsg ? "flex-end" : "flex-start"};margin-bottom:4px;` });
          if (msg.content && (msg.type === "user" || isChatReply(msg) || msg.type === "system" && msg.id.startsWith("msg_"))) {
            const isUser = msg.type === "user";
            const bubble = this.el("div", {
              style: `max-width:82%;
              background:${isUser ? `var(--ava-primary,${this.config.brandColor})` : "#ffffff"};
            ...
            if (!isUser) {
              const lbl = this.el("div", { style: "font-size:10px;color:#6366f1;font-weight:700;margin-bottom:3px;padding-left:4px;letter-spacing:0.04em;" });
              lbl.textContent = "AVA";
            ...
            wrapper.appendChild(bubble);
            {
              ...
              const d = new Date(ts);
              let h = d.getHours(), m2 = d.getMinutes();
              const ap = h >= 12 ? "PM" : "AM";
              h = h % 12 || 12;
              const tsEl = this.el("div", { style: `font-size:10px;color:#adb5bd;margin-top:2px;padding:0 4px;` });
              tsEl.textContent = `${h}:${String(m2).padStart(2, "0")} ${ap}`;
              wrapper.appendChild(tsEl);
            }
          }
          if (msg.payload?.products && msg.payload.products.length > 0) {
            const cardsWrap = this.el("div", { style: "display:flex;flex-direction:column;gap:8px;" });
        ...
        if (this.isTyping && this.messages.some((m) => m.type === "user")) {
          const typingWrap = this.el("div", { style: "display:flex;flex-direction:column;align-items:flex-start;padding:4px 0;" });
          const typingLbl = this.el("div", { style: "font-size:10px;color:#6366f1;font-weight:700;margin-bottom:3px;padding-left:4px;letter-spacing:0.04em;" });
          typingLbl.textContent = "AVA";
          const dots = this.el("div", { style: "display:flex;gap:4px;align-items:center;background:#ffffff;border-radius:18px 18px 18px 4px;padding:10px 14px;box-shadow:0 1px 2px rgba(0,0,0,0.08);" });
          [0,150,300].forEach(delay => {
            const d2 = this.el("div", { style: `width:6px;height:6px;border-radius:50%;background:#6366f1;animation:ava-typing 1s ${delay}ms infinite;` });
            dots.appendChild(d2);
          });
          typingWrap.appendChild(typingLbl);
    ...
    renderPanelView() {
      ...
      // ── Incremental update: reuse existing panel to avoid blink ───────────
      // The header is stable — only the three dynamic sections need refreshing.
      const existingLeadArea = this.panelContainer.querySelector("#ava-lead-area");
      const existingContentEl = this.panelContainer.querySelector("#ava-panel-content");
      const existingFooterEl  = this.panelContainer.querySelector("#ava-panel-footer");
      if (existingLeadArea && existingContentEl && existingFooterEl) {
        this._fillDynamicSections(existingLeadArea, existingContentEl, existingFooterEl);
        return;
      }
      // ── Full build (first expand) ─────────────────────────────────────────
      ...
      this._fillDynamicSections(leadArea, contentEl, footerEl);
    ...
    handleVoiceTranscript(transcript) {
      ...
      if (this.state === "expanded") {
        ...
        // Face-mic mode: stay minimized, just update the toggle badge
        this.renderToggle();
      }
    ...
    handleToggleClick() {
      if (this.state === "expanded") {
        ...
        if (this.currentNudge && !this.messages.some((m) => m.id === this.currentNudge?.intervention_id)) {
          ...
          if (n.voice_script && this.config.voiceAutoPlay !== false) welcomeScript = n.voice_script;
        }
        this.state = "expanded";
        this.currentNudge = null;
        this.hasUnread = false;
        ...
        // Play welcome voice if it hasn't been spoken yet this session
        if (!this._welcomeSpoken && this.config.voiceAutoPlay !== false) {
          this._welcomeSpoken = true;
          const welcomeMsg2 = "Hi, I am AVA. I am here to assist you with your shopping today. Just let me know if you need any assistance.";
          welcomeScript = welcomeMsg2;
        }
        if (welcomeScript) this.voiceManager?.speak(welcomeScript).catch(() => {});
      }
      this.render();
    }
    handleNudgeCtaClick() {
